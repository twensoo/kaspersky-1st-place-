...
def consumer_job(args, config):
    ...
    power_consumer = Consumer(config)

    # Set up a callback to handle the '--reset' flag.
    def reset_offset(power_consumer, partitions):
        if args.reset:
            for p in partitions:
                p.offset = OFFSET_BEGINNING
            power_consumer.assign(partitions)

    # Subscribe to topic
    topic = "power"
    power_consumer.subscribe([topic], on_assign=reset_offset)
    
    # Poll for new messages from Kafka and print them.
    try:
        while True:
            msg = power_consumer.poll(1.0)
