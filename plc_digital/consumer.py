...
def consumer_job(args, config, events_queue=None):
    ...
    plc_digital_consumer = Consumer(config)
    ...
    # Subscribe to topic
    topic = "plc_digital"
    plc_digital_consumer.subscribe([topic], on_assign=reset_offset)

    # Poll for new messages from Kafka and print them.
    try:
        while True:
            msg = plc_digital_consumer.poll(1.0)
            if msg is None:
                pass
            elif msg.error():
